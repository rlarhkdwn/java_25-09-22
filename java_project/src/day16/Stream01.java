...
public class Stream01 {
	...
	public static void main(String[] args) {
		...
		int[] arr = {1, 3 , 6, 1, 6, 5, 9, 6, 2, 7, 8, 6, 4, 2};
		int sum = 0;
		for (int i : arr) {
			sum += i;
		}
...
//		System.out.println(is.count()); // 이미 소모되어서 사용할 수 없음
		...
		// IntStream을 조건에 맞는 값만 추출하여 List 로 리턴
		...
		OptionalInt oi = Arrays.stream(arr).filter(n -> n > 5).findFirst();
